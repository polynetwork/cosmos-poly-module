...
func queryDenomInfo(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, error) {
	...
	if e != nil {
		return nil, sdkerrors.Wrapf(sdkerrors.ErrJSONMarshal, "could not marshal DenomInfo: %+v to JSON", denomInfo)
	}
...
func queryDenomInfoWithId(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, error) {
	...
	bz, e := codec.MarshalJSONIndent(types.ModuleCdc, *denomInfo)
	if e != nil {
		return nil, sdkerrors.Wrapf(sdkerrors.ErrJSONMarshal, "could not marshal DenomCrossChainInfo: %+v to JSON", denomInfo)
	}
