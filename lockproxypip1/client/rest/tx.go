...
func registerTxRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc(fmt.Sprintf("/lockproxypip1/create_and_delegate/{%s}/{%s}", Coin, LockProxyHash), CreateAndDelegateCoinRequestHandlerFn(cliCtx)).Methods("POST")
	r.HandleFunc(fmt.Sprintf("/lockproxypip1/bind_proxy/{%s}/{%s}", ToChainId, ToLockProxyHash), bindProxyRequestHandlerFn(cliCtx)).Methods("POST")
	r.HandleFunc(fmt.Sprintf("/lockproxypip1/bind_asset", ToChainId, ToLockProxyHash), bindAssetRequestHandlerFn(cliCtx)).Methods("POST")
	r.HandleFunc(fmt.Sprintf("/lockproxypip1/lock", ToChainId, ToLockProxyHash), lockRequestHandlerFn(cliCtx)).Methods("POST")
